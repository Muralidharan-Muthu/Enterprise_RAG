...
def _fixed_centroids(monkeypatch):
    centroids = {
        "table": np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32),
        "clause": np.array([0.0, 1.0, 0.0, 0.0], dtype=np.float32),
        "research": np.array([0.0, 0.0, 1.0, 0.0], dtype=np.float32),
        "vector": np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float32),
    }
...
def test_classify_confident_single_store():
    query_embedding = np.array([0.9, 0.1, 0.05, 0.05], dtype=np.float32)
...
def test_classify_ambiguous_low_similarity_searches_all_stores():
    query_embedding = np.array([0.1, 0.1, 0.1, 0.1], dtype=np.float32)
    out = sr.classify(query_embedding)
...
def test_classify_close_cluster_returns_multiple_stores():
    ...
    query_embedding = np.array([0.5, 0.48, 0.1, 0.1], dtype=np.float32)
    out = sr.classify(query_embedding)
    assert "table" in out["stores"]
    assert "clause" in out["stores"]
    assert "research" not in out["stores"]
