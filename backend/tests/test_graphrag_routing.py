"""route_graphrag() — Gemma-gated entity-intent routing.
...
period or a metric name). The decision is delegated to Gemma via
...
def _run(query, is_entity, entities=None, graph_hits=1):
    ...
    is_entity   → what the Gemma classifier returns
...
def test_factual_value_lookup_does_not_route_local():
    # "What is the revenue of FY 2023-24 - Planned?" — Gemma says NOT an entity
...
def test_entity_relationship_question_routes_local():
    # "Who is Kelvin He at Lenovo?" — Gemma says entity question AND the entity
...
def test_classifier_returns_false_when_gemma_unconfigured():
    with patch.object(gr.settings, "GEMMA4_BASE_URL", ""):
        assert gr._is_entity_query("Who is Kelvin He?") is False


def test_classifier_parses_graph_verdict():
    with patch.object(gr.settings, "GEMMA4_BASE_URL", "http://x"), \
         patch("app.services.gemma_client.chat", return_value='{"mode": "graph"}'):
        assert gr._is_entity_query("Who is Kelvin He?") is True


def test_classifier_parses_search_verdict():
    with patch.object(gr.settings, "GEMMA4_BASE_URL", "http://x"), \
         patch("app.services.gemma_client.chat", return_value='{"mode": "search"}'):
        assert gr._is_entity_query("What is the revenue of FY 2023-24?") is False


def test_classifier_defaults_false_on_gemma_error():
    with patch.object(gr.settings, "GEMMA4_BASE_URL", "http://x"), \
         patch("app.services.gemma_client.chat", side_effect=RuntimeError("boom")):
        assert gr._is_entity_query("Who is Kelvin He?") is False
